 ...
 * A&D meter
 */
public abstract class AAndDMeter extends Ble {
    ...
    public AAndDMeter(Activity activity, AAndDMeterListener listener) throws BleException {
        super(activity);
    ...
    protected void onBleConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        }else if(newState == BluetoothProfile.STATE_DISCONNECTED && mMode == MODE_READING){
            mListener.onConnectionStateChanged(Ble.FORCED_CANCEL);
            return;
        }
        ...
        if (BluetoothGatt.GATT_SUCCESS == status) {
            ...
            if (MODE_READING == mMode) {
                if(mListener!=null){
                    ...
                    Log.d(TAG,"SENDING on device bonded");
                    mListener.onDeviceBonded(mBluetoothDevice);
                }
                Log.d(TAG,"Reading mode, getFirmwareRevision");
                getFirmwareRevision(gatt);
                setDateTime(gatt);
                queueSetNotificationForCharacteristic(gatt.getService(BLOOD_PRESSURE_SERVICE).getCharacteristic
                               (BLOOD_PRESSURE_MEASUREMENT_CHARACTERISTIC),true);
    ...
    protected void onBleCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (FIRMWARE_REVISION_CHARACTERISTIC.equals(characteristic.getUuid())) {
           ...
           // disconnect();
        } else if (characteristic.getUuid()==WEIGHT_MEASUREMENT_CHARACTERISTIC){
            Log.d(TAG,"onBleCharacteristicRead - found weight char");
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!TextUtils.isEmpty(name)) {
                if (BluetoothDevice.BOND_BONDED == bondState) {
                    ...
                    Timber.d("Unreg Bonding Broadcast Receiver");
