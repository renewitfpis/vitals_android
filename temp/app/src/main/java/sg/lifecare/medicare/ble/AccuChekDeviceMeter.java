...
import sg.lifecare.medicare.ble.profile.GlucoseMeasurementParser;
 ...
 */
public class AccuChekDeviceMeter extends Ble implements ServiceConnection {

    private static final String TAG = "AccuChekDeviceMeter";
    ...
    public AccuChekDeviceMeter(Activity activity, AAndDMeterListener listener, Model deviceModel, String deviceId) throws BleException {
        super(activity);
    ...
    public AccuChekDeviceMeter(Activity activity, AAndDMeterListener listener, Model deviceModel, ArrayList<String> deviceIds) throws BleException {
        super(activity);
    ...
    public AccuChekDeviceMeter(Activity activity, AAndDMeterListener listener, Model deviceModel) throws BleException {
        super(activity);
    ...
    public AccuChekDeviceMeter(Activity activity, AAndDMeterListener listener, int type) throws BleException {
        super(activity);
    ...
    public void initService(){
        ...
        }else if(deviceModel==Model.ACCU_CHEK_AVIVA_CONNECT){
            SERVICE = GLUCOSE_SERVICE;
            CHARACTERISTIC = GLUCOSE_CHARACTERISTIC;
        }
    }
        ...
        if(pairingMode == PAIRING_MODE_ID){
            ...
            if(deviceModel!=null) {
                ...
                if (!TextUtils.isEmpty(name)) {
                    ...
                    if (name.toLowerCase().contains(nameToPair.toLowerCase())) {
                        return true;
                    }
                }
            }else{
                ...
                if (!TextUtils.isEmpty(name)) {
                    ...
                    if(type==TYPE_SPO2) {
                        ...
                        if (name.startsWith("Accu-Chek"))
                        {
                            deviceModel = Model.ACCU_CHEK_AVIVA_CONNECT;
                            initService();
                            return true;
                        }
                    }
    ...
    public void parseProfile(BluetoothGattCharacteristic cha){
        ...
        if(deviceModel==Model.BERRY_BM1000B||deviceModel==Model.NONIN_3230) {
            ...
            Log.d(TAG, "ACCU-CHEK AVIVA CHAR");
            if(cha.getUuid().equals(CHARACTERISTIC)) {
                Log.d(TAG, "ACCU-CHEK AVIVA GLUCOSE MEASUREMENT CHAR");
                Terumo terumo = GlucoseMeasurementParser.parse(cha);
                if(terumo!=null && mListener!=null){
                    mListener.onDataRetrieved(terumo);
                }
            }
        }
        ...
        if (GATT_ERROR == status) {
            ...
            if (MODE_READING == mMode) {
                ...
                if (BluetoothDevice.BOND_NONE == mBluetoothDevice.getBondState()||
                    ...
                    //for accu-chek, need to explicitly call createBond()
                    if (VERSION.SDK_INT >= VERSION_CODES.KITKAT) {
                        Timber.d("CREATE BOND");
                        boolean bonding = mBluetoothDevice.createBond();

                        Timber.d("CREATE BOND222 " + bonding);
                        Activity act = mActivity.get();
                        IntentFilter filter = new IntentFilter(BluetoothDevice.ACTION_BOND_STATE_CHANGED);
                        act.registerReceiver(mBondingBroadcastReceiver, filter);
                    }
    ...
    protected void onBleServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (MODE_READING == mMode) {
            ...
            if(mBluetoothGatt.getService(SERVICE)!=null) {
                Timber.d("FOUND SERVICE, set notification");
        ...
        if (BluetoothGatt.GATT_SUCCESS == status) {
            if ((MODE_PAIRING == mMode) && (BluetoothDevice.BOND_BONDED != mBluetoothDevice.getBondState())){
                ...
                Log.d(TAG,"Pairing mode, wait for bonded broadcast");
                return;
            }else if ((MODE_PAIRING == mMode) && (BluetoothDevice.BOND_BONDED == mBluetoothDevice.getBondState()))
            {
                Log.d(TAG,"Pairing mode, already bonded");
            }

            if (MODE_READING == mMode) {
                Log.d(TAG,"Reading mode, getFirmwareRevision");

                queueSetNotificationForCharacteristic(gatt.getService(SERVICE).getCharacteristic
                        (CHARACTERISTIC),true);
                queueSetNotificationForCharacteristic(gatt.getService(SERVICE).getCharacteristic
                        (GLUCOSE_CONTEXT_CHARACTERISTIC),true);
                queueSetIndicationForCharacteristic(gatt.getService(SERVICE).getCharacteristic
                        (GLUCOSE_RECORD_ACCESS_CHARACTERISTIC),true);
                accessGlucoseRecord();

                Log.d(TAG,"write to record access control point");

                //mBluetoothGatt.setCharacteristicNotification(gatt.getService(SERVICE).getCharacteristic
                  //            (CHARACTERISTIC),true);
                Log.d(TAG,"set notification done");
    ...
    protected void accessGlucoseRecord() {
        //write to Record Access Control Point
        BluetoothGattService glucoseService = mBluetoothGatt.getService(GLUCOSE_SERVICE);

        if(glucoseService != null){
            Timber.d("Glucose Service is not null!");
            BluetoothGattCharacteristic writeRACPchar = glucoseService
                    .getCharacteristic(GLUCOSE_RECORD_ACCESS_CHARACTERISTIC);
            ...
            // or:
             //byte[] data = new byte[2];
             //writeRACPchar.setValue(data);
             //writeRACPchar.setValue(1,BluetoothGattCharacteristic.FORMAT_UINT8, 0); // Report Stored records
             //writeRACPchar.setValue(1,BluetoothGattCharacteristic.FORMAT_UINT8, 1); // Read all (offset 1)
             //writeRACPchar.setValue(0,BluetoothGattCharacteristic.FORMAT_UINT8, 2);

            queueWriteDataToCharacteristic(writeRACPchar,data);
            //boolean success = mBluetoothGatt.writeCharacteristic(writeRACPchar);
            //Timber.d("Written RACP: " + success);
        }
    ...
    protected void onBleCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        }else if(characteristic.getUuid().equals(CHARACTERISTIC)){
            Timber.d("Glucose char found!!");
        }
    }
       ...
       /* BluetoothGattService service = gatt.getService(SERVICE);
        ...
        for(int i = 0; i < serviceList.size(); i++) {
            ...
            for(int j=0; j < characteristicList.size(); j++){
                ...
                if(characteristicList.get(j).getUuid() == CHARACTERISTIC){
                    queueSetNotificationForCharacteristic(characteristic,true);
                }
            }
        }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!TextUtils.isEmpty(name)) {
                if (BluetoothDevice.BOND_BONDED == bondState) {
                    ...
                    Timber.d("Is Bonding");
                    //do nothing, wait for bonded broadcast
                } else{
                    Timber.d("Unregister Bonding Broadcast Receiver");
                    context.unregisterReceiver(mBondingBroadcastReceiver);
                }
